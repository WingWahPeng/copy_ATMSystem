...
public class ATMWait extends ATMPanel {
								...
								Entrance.getDBInstance().executeQuery(
										...
										new OnDBProcessingFinish() {
											...
											public void onFinish(Object j) {
												...
												try {
													if (rs2.next()) {
														cuInfo = new CustomerInformation();
													...
													MyUtil.setCustomerInformation(cuInfo);
													MyUtil.panelLeap(
															ATMWait.this,
															new MainWindow(),
															null);
				...
				public void onFinish(Object j) {
					...
					try {
						if(rs.next()){
							...
							//先检查是否为中行账户登录，为外行登录则再检查用户单击的按钮类型
							if(MyUtil.getCardInformation().getBelong()==2||TransferAccount.flag==cInfo1.getBelong()){
								//1 "正常"，2 "冻结"，3 "挂失"，4 "注销"
								...
								if(cInfo1.getBelong()==1){
									
									MyUtil.setProblem("您输入的账号为本行，请重新输入或选择行内转账！");
									
								}else{
									
									MyUtil.setProblem("您输入的账号非本行，请重新输入或选择跨行转账！");
	...
	public void update() {
		...
		Entrance.getDBInstance().executeUpdate(sqls,
				new OnDBProcessingFinish() {
