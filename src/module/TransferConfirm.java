...
public class TransferConfirm extends ATMPanel {
	...
	private JButton returnCard, yesButton;//退卡，确认
	private JLabel tips, tips1, tips2,tips3;//右上角提示，输入提示，中下提示，错误提示
	private ATMTextField transferPassword;//转账密码验证
	...
	public TransferConfirm() {
		...
		add(returnCard);
		returnCard.addActionListener(new ActionListener() {//添加退卡事件监听
		...
		add(yesButton);
		yesButton.addActionListener(new ActionListener() {//添加动作事件监听
			public void actionPerformed(ActionEvent arg0) {
		...
		transferPassword.addKeyListener(new KeyAdapter(){//添加键盘事件监听
