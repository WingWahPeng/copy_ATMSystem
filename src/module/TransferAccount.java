...
public class TransferAccount extends ATMPanel implements ActionListener {
	...
	private JButton returnCard, inlineTransfer, interBankTransfer,//退卡，行内转转，行外转账
			backMainWindow;//返回主菜单
	private JLabel tips, tips1;
	private Clocker clocker;
	public static int flag;//卡行内外转账标识
	...
	public TransferAccount() {
		...
		if(MyUtil.getCardInformation().getBelong()!=1){
			tips1.setText("由于您使用非中行账户登录，不能进行转账!");
			inlineTransfer.setEnabled(false);
			interBankTransfer.setEnabled(false);
		}
	}
//只有本行的卡在本行的ATM机，才可以进行转账
