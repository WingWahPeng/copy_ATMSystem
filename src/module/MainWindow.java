...
public class MainWindow extends ATMPanel implements ActionListener {
	...
	private JButton check, changePW, transferAccount, withdrawal, deposit,returnCard;
	//查询，改密，转账，取款，存款，退卡
	...
	public MainWindow() {
		...
		add(clocker);
		clocker.startCountDown();//启动倒计时线程
		...
		name = MyUtil.getCustomerInformation().getCustomerName().substring(0, 1);
	...
	private void checkCardStatus() {
		if(MyUtil.getCardInformation().getBelong()!=1){//修正其他银行的银行卡所能进行的操作
		...
		case 2:
			...
			userInformation
					.setText("<html><center>"+name+"先生，您的账号已被冻结！</p></html>");
